...
class ExampleWindow(Adw.ApplicationWindow):
    def __init__(self, **kwargs):
        ...
        for n in range(1, 4):
            adw_button_row = Adw.ButtonRow.new()
            adw_button_row.set_title(title=f"Adw.ButtonRow {n}")
            adw_button_row.set_start_icon_name(icon_name="go-previous-symbolic")
            adw_button_row.set_end_icon_name(icon_name="go-next-symbolic")
            adw_button_row.connect("activated", lambda x: print("Clicked"))
            list_box.append(child=adw_button_row)
